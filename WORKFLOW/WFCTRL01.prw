...
*/
********************************
User Function WFCTRL01(_cRecList)
********************************
Local aEnvia := {}
Local cProd,cMen
Local lRet := .T.
...
//
If Len(aEnvia) > 0
	oProcess := TWFProcess():New( "000027", "WFCTRL01 - ITENS APROVADOS")
	oProcess :NewTask( "100027", "\WORKFLOW\WFCTRL01.HTM" )
	oProcess :cSubject := "WFCTRL01 - Monitoramento de perdas aprovadas"
	...
	cMen += ' <td colspan=15 >Relação de Lotes com apontamento de perda aprovada no periodo '+dToc(dDataIni)+' a '+dToc(dDataFim)+' </td></tr>'
	cMen += ' <tr >'
