...
*/
********************************
User Function WFCTRL02(_cRecList)
********************************
Local aEnvia := {}
Local cProd,cMen
Local lRet := .T.
...
cQuery += " AND A.R_E_C_N_O_ IN ("+_cRecList+") "
cQuery += " AND ZZ6_FILIAL = '"+xFilial("ZZ6")+"' "
cQuery += " AND ZZ6_STATUS = 'R'"
...
Do While ZZ6WFT->(!Eof())
	...
	Do While ZZ6WFT->(!Eof()) .AND. ZZ6WFT->ZZ6_LOCAL == cLocal
		...
		//MSGINFO(ZZ6WFT->ZZ6_PRODUT+ZZ6WFT->ZZ6_LOCORI+DTOS(dDatabase))
		//MSGINFO(STR(ASALDO[2])+" "+STR(ASALDO[1]))
		//
...
//
If Len(aEnvia) > 0
	oProcess := TWFProcess():New( "000028", "WFCTRL02 - ITENS REJEITADOS")
	oProcess :NewTask( "100028", "\WORKFLOW\WFCTRL02.HTM" )
	oProcess :cSubject := "WFCTRL02 - Monitoramento de perdas com aprovação declinada"
	...
	cMen += ' <td colspan=15 >Relação de Lotes com apontamento de Perda com aprovação declinada no periodo '+dToc(dDataIni)+' a '+dToc(dDataFim)+' </td></tr>'
	cMen += ' <tr >'
