...
//
IF Dow(ddatabase) <> 2
   Conout("O dia não é uma segunda-feira!")
   Return
Endif
//
cDataIni := dDatabase - 7
cDataFim := dDatabase - 3
//
dbSelectarea("ZZ6")
...
cQuery += " AND (ZZ6_DTAUT  <> '' AND ZZ6_DTCQL  <> '') "
cQuery += " AND (    (ZZ6_DTAUT >= ZZ6_DTCQL AND ZZ6_DTAUT BETWEEN '"+cDataIni+"' AND '"+cDataFim+"') "
cQuery += "       OR (ZZ6_DTCQL >= ZZ6_DTAUT AND ZZ6_DTCQL BETWEEN '"+cDataIni+"' AND '"+cDataIni+"') )
...
Do While ZZ6WFT->(!Eof())                                      
	    ...
	    //       
	    nCusItem := 0
		aSaldo   := CalcEst(ZZ6WFT->ZZ6_PRODUT, ZZ6TMP->ZZ6_LOCORI, dDatabase+1)
		...
		aadd(aEnvia, {dToc(ZZ6WFT->ZZ6_DTRM)+" "+ZZ6WFT->ZZ6_HORA,; 
			               ...
			               Posicione("ZZ2",1,xFilial("ZZ2")+ZZ6WFT->ZZ6_CODMOT,"ZZ2_DESCRI"),;
		   	               Alltrim(ZZ6WFT->ZZ6_OBS),;
		   	               ZZ6ChkStatus(ZZ6WFT->ZZ6_STATUS) })
		//      
		nCusLoc += nCusItem   
 		//
		ZZ6WFT->(dbSkip())
...
//
If Len(aEnvia) > 0	
	oProcess := TWFProcess():New( "000029", "WFCTRL03 - ITENS APROVADOS")
	oProcess :NewTask( "100029", "\WORKFLOW\WFCTRL03.HTM" )
	oProcess :cSubject := "WFCTRL03 - Monitoramento de Perdas no processo"
	...
	cMen += ' <td colspan=15 >Relação de Lotes com apontamento de Perda APROVADOS</td></tr>'
	cMen += ' <tr >'
